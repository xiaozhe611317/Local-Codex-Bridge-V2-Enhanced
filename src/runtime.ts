...
export class RuntimeStore {
  ...
  async waitForSettingsUpdated(threadId: string, afterCursor: number, timeoutMs = 5_000): Promise<RuntimeEvent> {
    const runtime = this.#threads.get(threadId);
    if (!runtime) throw new Error("Settings update thread is not loaded");
    const deadline = Date.now() + timeoutMs;
    while (true) {
      const revision = runtime.revision;
      const event = runtime.events.find(e => e.cursor > afterCursor && e.method === "thread/settings/updated");
      if (event) return event;
      const remaining = deadline - Date.now();
      if (remaining <= 0) throw new Error("UNKNOWN: settings update may be accepted; confirmation missing. Observe/read before retrying.");
      await this.#waitForChange(runtime, revision, remaining);
    }
  }
